...
def generate_filename(yt_obj, idx, is_playlist=False, numbering=False):
    ...
    try:
        ...
        if is_playlist and numbering:
            ...
            f_name = r"%s" % f_name
        return f_name
    except: raise FilenameError("Could not get filename")


def rename_file(string):
    """catch file naming conventions from youtube to avoid clash with directory
    and file naming convention in windows[Errno 2 error]"""
    invalid_characters = {"<": "",
                          ">": "",
                          "|": "-",
                          ":": " -",
    ...
    for i, j in invalid_characters.items():
